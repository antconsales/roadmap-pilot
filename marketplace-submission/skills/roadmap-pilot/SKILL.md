...
description: >
  ...
  done, commits, and hands off to a new session. Works with any cleanup task:
  typing, refactoring, migration, renaming. One task per session, zero hallucinations.
  Use when the user says "continue the roadmap", "next task", "roadmap", or similar.
  Also activates when the user types /roadmap.
  Companion skill: init-roadmap (generates the CLAUDE.md roadmap).
user-invocable: true
...
> **Roadmap completed!** All tasks have been executed.
...
| Script | What it does |
|--------|-------------|
| `scripts/status.sh` | Visual progress dashboard with bars per phase |
| `scripts/next-task.sh` | Shows next task and progress count |
| `scripts/auto-roadmap.sh` | Full autopilot loop (runs `claude -p` repeatedly) |
| `scripts/rollback.sh` | Safely reverts the last roadmap commit |

## Edge cases

- **No `CLAUDE.md` found**: Tell the user to run `/init-roadmap` first or create one manually
...
- Bug fixing (systematic batch fixes)
